...
Clicking on the Test Rule element in the navigator window opens an interface in 
the work window and interacts with the /rules/rule-check end point of Eiffel 
Intelligence. This interface can be used to test rules on events.
...
is intended to load the rule. To do this one will need to chose the rule type 
...
identifier. If the identifier does not exist in the JSON document, then a null 
...
<code>a.b.c.d</code>. <a href="http://jmespath.org/specification.html#identifiers">JMESPath identifier documentation</a>.</p>
...
value is a JMESPath identifier. A separate rule is created for each event type 
that is going to take part in the creation of the aggregated object. This means 
that if you want to create your aggregated object from 3 event types and then 
all other event types are ignored. Each rule might contain some of the keys 
listed below:
</p>
...
our aggregated object. Keep in mind that we are adding to the root of aggregated 
...
<code>"IdentifyRules":"links | [?type=='SUBJECT'].target"</code>. At this point 
we know the needed aggregated object id and now we need to specify the desired 
location for the confidence level information. Lets move the data obtained in 
"ExtractionRules" under the <code>"confidenceLevels"</code> key in the root of
the aggregated object and the value should be an array. The identifier is 
expressed as <code>{confidenceLevels: [{ eventId: meta.id }]}</code>. To insert 
the new element to an array we need to provide an expression that won't be found 
in the existing aggregated object (if the provided identifier is found in the 
...
<h4>Updating an array in JSON objects where the a field is pointing to Eiffel 
message id</h4>
...
<p>
Downstream events in Eiffel means those events that will be issued after or 
were the outcome to the specified event. Such a connection works via the 
<a href="https://github.com/Ericsson/eiffel/blob/master/eiffel-syntax-and-usage/the-links-object.md">links</a>
mechanism. For example we have EiffelSourceChangeCreatedEvent,
EiffelSourceChangeSubmittedEvent, EiffelArtifactCreatedEvent, 
